...
class AttModel(CaptionModel):
    def __init__(self, args, tokenizer):
        ...
        self.att_embed_con = nn.Sequential(*(
                ((nn.BatchNorm1d(512),) if self.use_bn else ()) +
                (nn.Linear(512, self.input_encoding_size),
                 nn.ReLU(),
                 nn.Dropout(self.drop_prob_lm)) +
                ((nn.BatchNorm1d(self.input_encoding_size),) if self.use_bn == 2 else ())))
    ...
    def _sample_beam(self, fc_feats, att_feats, att_feats_con, att_masks=None, att_masks_con=None, opt={}):
        ...
        p_fc_feats, p_att_feats, pp_att_feats, p_att_masks = self._prepare_feature(fc_feats, att_feats, att_feats_con, att_masks, att_masks_con)
    ...
    def _sample(self, fc_feats, att_feats, att_feats_con, att_masks=None, att_masks_con=None):
        ...
        if beam_size > 1 and sample_method in ['greedy', 'beam_search']:
            return self._sample_beam(fc_feats, att_feats, att_feats_con, att_masks, att_masks_con, opt)
        if group_size > 1:
            return self._diverse_sample(fc_feats, att_feats, att_feats_con, att_masks, att_masks_con, opt)
    ...
    def _diverse_sample(self, fc_feats, att_feats, att_feats_con, att_masks=None, att_masks_con=None, opt={}):
