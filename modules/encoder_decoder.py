...
class Transformer(nn.Module):
    ...
    def forward(self, src, src_con, tgt, src_mask, src_mask_con, tgt_mask):
        return self.decode(self.encode(src, src_con, src_mask_con), src_mask, tgt, tgt_mask)

    def encode(self, src, src_con, src_mask_con):
        return self.encoder(self.src_embed(src), self.src_embed(src_con), src_mask_con)
...
class Encoder(nn.Module):
    ...
    def forward(self, x, x_con, mask):
        for layer in self.layers:
            x = layer(x, x_con, mask)
        return self.norm(x)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, x_con, mask):
        x = self.sublayer[0](x, lambda x: self.self_attn(x, x_con, x_con, mask))
...
class EncoderDecoder(AttModel):
    ...
    def _prepare_feature(self, fc_feats, att_feats, att_feats_con, att_masks, att_masks_con):

        att_feats, att_feats_con, seq, att_masks, att_masks_con, seq_mask = self._prepare_feature_forward(att_feats, att_feats_con, att_masks, att_masks_con)
        memory = self.model.encode(att_feats, att_feats_con, att_masks_con)

        return fc_feats[..., :1], att_feats[..., :1], memory, att_masks

    def _prepare_feature_forward(self, att_feats, att_feats_con, att_masks=None, att_masks_con=None, seq=None):
        att_feats, att_masks = self.clip_att(att_feats, att_masks)
        att_feats_con, att_masks_con = self.clip_att(att_feats_con, att_masks_con)
        att_feats = pack_wrapper(self.att_embed, att_feats, att_masks)
        att_feats_con = pack_wrapper(self.att_embed_con, att_feats_con, att_masks_con)

        if att_masks is None:
            att_masks = att_feats.new_ones(att_feats.shape[:2], dtype=torch.long)
        if att_masks_con is None:
            att_masks_con = att_feats_con.new_ones(att_feats_con.shape[:2], dtype=torch.long)
        att_masks = att_masks.unsqueeze(-2)
        att_masks_con = att_masks_con.unsqueeze(-2)
        ...
        return att_feats, att_feats_con, seq, att_masks, att_masks_con, seq_mask

    def _forward(self, fc_feats, att_feats, att_feats_con, seq, att_masks=None, att_masks_con=None):

        att_feats, att_feats_con, seq, att_masks, att_masks_con, seq_mask = self._prepare_feature_forward(att_feats, att_feats_con, att_masks, att_masks_con, seq)
        out = self.model(att_feats, att_feats_con, seq, att_masks, att_masks_con, seq_mask)
